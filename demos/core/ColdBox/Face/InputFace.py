...
class Face:
    ...
    def __init__(self) -> None:
        ...
        self.cnn_detector = dlib.cnn_face_detection_model_v1('./face_weights/mmod_human_face_detector.dat')
        self.haar_face_detector  = cv2.CascadeClassifier('./face_weights/haarcascade_frontalface_default.xml')

        # 加载关键点检测器
        self.points_detector = dlib.shape_predictor('./face_weights/shape_predictor_68_face_landmarks.dat')
        # 加载resnet模型
        self.face_descriptor_extractor = dlib.face_recognition_model_v1('./face_model/dlib_face_recognition_resnet_model_v1 .dat')
        self.recognizedName = None #已识别出的人
    ...
    def faceRegiser(self,faceId=1,userName='default',interval=3,faceCount=3,resize_w=0,resize_h=0):
        ...
        # 控制显示打卡成功的时长
        # 打开文件
        f =  open('./face_data/feature.csv','a',newline='')
...
# 获取CSV中信息
    def getFeatList(self):
        ...
        name_list = []
        # 加载保存的特征样本
        with open('./face_data/feature.csv','r',encoding = 'utf-8') as f:
    ...
    # 人脸识别
    def faceRecognize(self,detector='haar',threshold=0.5):
        ...
        # 考勤记录
        f = open('./face_data/attendance.csv','a',encoding = 'utf-8')
        csv_writer = csv.writer(f)
        ...
        while True:
            ...
            for face in face_detetion:
                ...
                if min_distance < threshold:
                    ...
                    # 判断是否新增记录：如果一个人距上次检测时间>60秒，或者换了一个人，将这条记录插入
                    need_insert = False
                    now = time.time()
                    if predict_name in face_time_dict:
                        if (now - face_time_dict[predict_name]) > 60:
                            # 刷新时间
                ...
                self.recognizedName = predict_name
    ...
    def discernFace(self):
        ...
        parser.add_argument("--threshold", type=float, default=0.5,
                                        help="人脸识别距离阈值，越低越准确")
        args = parser.parse_args()
        self.faceRecognize(detector=args.detector, threshold=args.threshold)



if __name__ == '__main__':
    face = Face()
    face.inputFace('刘峰','193080100')
    # face.discernFace()
